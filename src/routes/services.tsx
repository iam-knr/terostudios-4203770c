...
function ServicesPage() {
  // Render the sub-route page (services/$slug) when one is active, otherwise the hub.
  const isLeaf = typeof window !== "undefined" && window.location.pathname !== "/services";
  return isLeaf ? <Outlet /> : <ServicesHub />;
}
    ...
    <PageLayout>
      ...
      {Object.entries(grouped).map(([category, list], idx) => (
        <section
          key={category}
          className={
            idx % 2 === 0 ? "border-y border-parchment bg-card" : ""
          }
        >
