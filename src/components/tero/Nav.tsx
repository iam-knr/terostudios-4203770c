...
function CyclingWord({ onDark }: { onDark: boolean }) {
  ...
  return (
    <span className="relative inline-flex items-center h-7 md:h-8 min-w-[110px] md:min-w-[130px] overflow-visible leading-none ml-2">
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className={[
            "absolute left-0 top-1/2 -translate-y-1/2 whitespace-nowrap transition-colors",
            onDark ? "text-white" : "text-ink",
          ].join(" ")}
        >
          <span className="tero-logo-cycle-text text-[22px] md:text-[26px]">{cyclingWords[i]}</span>
        </motion.span>
      </AnimatePresence>
    </span>
  );
