...
function MarqueeRow({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (!pausedRef.current) {
        ...
        if (direction === "right" && offsetRef.current >= third) offsetRef.current -= third;
  ...
  // pre-offset right-moving rows so they start visible
  useEffect(() => {
    if (direction === "right" && trackRef.current) {
      offsetRef.current = -trackRef.current.scrollWidth / 3;
    }
  }, [direction]);
  ...
  return (
    ...
    >
      <div
        ref={trackRef}
        className="flex w-max items-center gap-10 md:gap-16 will-change-transform py-2"
      >
        {loop.map((p, i) => (
          <div key={i} className="flex items-center gap-10 md:gap-16 shrink-0">
            <div className="group flex flex-col items-center text-center opacity-70 hover:opacity-100 transition-opacity duration-300">
              <h3 className="font-sans-display font-bold uppercase tracking-tight text-[clamp(40px,6.5vw,96px)] leading-[0.9] text-ink whitespace-nowrap">
                {p.name}
              </h3>
              <div className="mt-2 flex items-center gap-3 font-body text-[11px] md:text-[13px] text-slate">
                <span>{p.role}</span>
                <a
                  href={p.li ?? "#"}
                  target="_blank"
...
export function TeamMarquee({
  ...
  return (
    <section className="relative overflow-hidden bg-cream text-ink">
      ...
      <div className="relative z-10 mt-12 md:mt-16 pb-20 md:pb-28">
