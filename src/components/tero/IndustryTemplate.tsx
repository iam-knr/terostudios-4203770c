...
export function IndustryTemplate({ industry }: { industry: IndustryEntry }) {
  return (
    <PageLayout>
      ...
      <section className="border-y border-parchment bg-card">
        <div className="container-tero grid grid-cols-1 gap-12 py-20 md:grid-cols-12 md:py-28">
          <div className="md:col-span-5">
            <p className="overline">— The pain</p>
          ...
          <ul className="md:col-span-7 space-y-5">
            {industry.pain.map((p) => (
              <li key={p} className="flex gap-4 font-body text-[16px] leading-relaxed text-slate">
                <AlertCircle className="mt-1 h-5 w-5 shrink-0 text-vermillion" strokeWidth={1.5} />
                {p}
              </li>
            ))}
          </ul>
        ...
        <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {industry.services.map((s, i) => (
            <Reveal key={s} delay={i % 3}>
              <div className="rounded-2xl border border-parchment bg-card p-7">
                <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-vermillion">
                  — {String(i + 1).padStart(2, "0")}
                </p>
                <p className="mt-4 font-sans-display text-[20px] font-bold text-ink">{s}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="border-y border-parchment bg-card">
        <div className="container-tero py-20 md:py-28">
          <p className="overline">— In good company</p>
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            {industry.proof.map((p) => (
              <span
                key={p}
                className="rounded-full border border-parchment bg-cream px-5 py-2.5 font-sans-display text-[14px] font-bold tracking-[0.1em] text-ink/70"
              >
                {p}
              </span>
            ))}
      ...
      <section className="bg-ink text-cream">
        <div className="container-tero grid grid-cols-1 gap-10 py-20 md:grid-cols-12 md:py-28">
          <div className="md:col-span-8">
            <p className="overline text-vermillion">— Let&apos;s talk</p>
            <h2 className="mt-4 hero-headline text-[clamp(36px,6vw,72px)] text-cream">
              Brief us on your {industry.name.toLowerCase()} brief.
            </h2>
          </div>
